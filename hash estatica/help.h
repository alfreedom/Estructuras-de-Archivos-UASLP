...
void GenerateHelp()
{
    FILE *f=fopen("HELPSTATIC","w");

    if(f)
    {
        fprintf(f,"**************** AYUDA DE LA ORGANIZACIÖN HASH ESTATICA *****************\n\n");
        fprintf(f,"NOMBRE:\n");
        fprintf(f,"    hestatica (linux)   Organización Hash Estatica\n\n");
        fprintf(f,"SINTAXYS:\n");
        fprintf(f,"    hestatica <opncion1> parametro <opcion2> parametro ...\n\n");
        fprintf(f,"DESCRIPCION:\n");
        fprintf(f,"    Crea y  manipula  registros de un diccionario con claves Has de manera\n");
        fprintf(f,"    estatica, agregando, eliminando, modificando y consultado registros\n\n");
        ...
        fprintf(f,"    ALTA:     -d dic.hs -a TABLA\n");
        fprintf(f,"    BAJA:     -d dic.hs -b TABLA CLAVE\n");
        fprintf(f,"    CONSULTA: -d dic.hs -c (consulta todas las tablas)\n");
        fprintf(f,"    CONSULTA: -d dic.hs -c TABLA (consulta una tabla)\n");
        fprintf(f,"    MODIFICA: -d dic.hs -m TABLA CLAVE\n\n");
