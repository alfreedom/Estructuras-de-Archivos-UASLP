...
HashEstatica::HashEstatica(char* nombreDic) {
    
    _nomdic=new char[strlen(nombreDic)+1];
    strcpy(_nomdic,nombreDic);
    _arch=new Archivo(nombreDic);
    _cons=new Consola();
    _cons->SetPrompt("HashEstatica>:");
    _cons->Init();
}

HashEstatica::HashEstatica(const HashEstatica& orig) {
}

HashEstatica::~HashEstatica() {
    _cons->Exit();
}

int HashEstatica::ImportaDiccionario(char* nombreDic, char* nombreSec){
    ...
    if( dicSec->creaArchivo() ){
        ...
        printf("Error: No se pudo creal el diccionario HashEstatica \"%s\"\n",nombreSec);
    
    return ret;
}
int HashEstatica::AltaHashEstatica(char* nomEntidad){
   ...
   Entidad* ent;
    ...
    if(ent=buscaEnt(nomEntidad))
    {
        ...
        if(ent->getDirAtributos()!=EOF){            
            ...
            DIR_t posind;
            Indice* ind=dameCubeta(ent,reg,&posind);
            //if(numElemtosIndice(ent,ind)<RANGO)
            {
...
int HashEstatica::BajaHashEstatica(char* nomEntidad, char* key){
    ...
    if(ent=buscaEnt(nomEntidad))
    {
        ...
        DIR_t posi;
        Indice *ind=dameCubeta(ent,reg,&posi);
        printw("\n*** Baja HashEstatica: ***");
...
int HashEstatica::ModificaHashEstatica(char* nomEntidad, char* key){
    ...
    if(ent=buscaEnt(nomEntidad))
    {
        ...
        if(aux=buscaReg(ent,reg,&posreg,ind)){
           ...
           DIR_t posindnew;
           Indice* indnew=dameCubeta(ent,nuevo,&posindnew);
...
int HashEstatica::ConsultaHashEstatica(char* nomentidad){
...
Registro* HashEstatica::pideRegistro(Entidad* ent){
...
Registro* HashEstatica::pideRegistroFrom(Entidad* ent,Registro* reg){
...
void* HashEstatica::pideDato(Atributo* atr){
...
void* HashEstatica::pideDatoFrom(Atributo* atr, void* dato){
...
int HashEstatica::esEntero(char* cad){
...
int HashEstatica::esDecimal(char* cad){
...
int HashEstatica::insertaRegistro(Entidad* ent,DIR_t posEnt, Registro* reg,DIR_t posReg, Indice* indice, DIR_t posind){
...
DIR_t HashEstatica::eliminaRegistro(Entidad* ent,DIR_t posEnt, Registro* reg,Indice* indice, DIR_t posind){
...
int HashEstatica::buscaRegistro(Entidad* ent, Registro* reg,Indice* indice){
    DIR_t p;
     return buscaReg(ent,reg,&p,indice)!=NULL;
}
Registro* HashEstatica::buscaReg(Entidad* ent, Registro* reg, DIR_t *pos,Indice* indice){
...
Entidad* HashEstatica::buscaEnt(char* nombre){
...
int HashEstatica::buscaEntidad(char* nombre){

	return (this->buscaEnt(nombre) != NULL);
        
}
int HashEstatica::buscaEntidad(Entidad* entidad){

    return this->buscaEntidad(entidad->getNombre());
}

DIR_t HashEstatica::getPosEnt(char* NombreEntidad){
    DIR_t  cab= _arch->leeCab();
...
int HashEstatica::numElemtosIndice(Entidad* e,Indice* ind){
    
    DIR_t cabdatos= ind->getPtrDatos();
    int count=0;
    
    while(cabdatos!=EOF){
        Registro* reg= new Registro(e,_arch);
        reg->setBuffer(_arch->leeBloque(reg->getSize(),cabdatos));
        count++;
        cabdatos=reg->getSiguiente();
    }
...
Indice* HashEstatica::dameCubeta(Entidad* ent, Registro* reg, DIR_t *posIndice){
...
int HashEstatica::Hash(Entidad* ent, Registro* reg){
    Atributo* atr= getAtrPrimario(ent);
    DIR_t pin;
    Indice* ind=buscaIndice(ent,reg,atr,&pin);
    int clave;
    if(!ind){
...
Indice* HashEstatica::buscaIndice(Entidad* ent, Registro* reg, Atributo* atrPrimario, DIR_t *posIndice){
...
void HashEstatica::InsertaIndice(Entidad* ent,DIR_t posent,Indice* ind, DIR_t posind){
...
Atributo* HashEstatica::getAtrPrimario(Entidad* ent){
    DIR_t cabatr=ent->getDirAtributos();
    Atributo* atr=NULL;
    while(cabatr!=EOF){
