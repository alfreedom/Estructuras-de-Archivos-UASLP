...
int main(int argc, char** argv) {
    ...
    if(argc>1){
        ...
        if(strcmp(ops.importa,"")){
            Secuencial::ImportaDiccionario(ops.importa,ops.outputdic);
        }
        else
        if(strcmp(ops.dic,"")){
            sec=new Secuencial(ops.dic);
            
            if(strcmp(ops.alta,""))
            {
                
                _error_=sec->AltaSecuencial(ops.alta);
                if(!_error_){
                    sec->ConsultaSecuencial(ops.alta);                
                    getch();
                }
            }
            ...
            {                
                _error_=sec->BajaSecuencial(ops.baja,ops.key);
                
                if(!_error_)
                {
                    sec->ConsultaSecuencial(ops.baja);
            ...
            if(strcmp(ops.consulta,""))
            {
                if(!strcmp(ops.consulta,"ALL"))
                    _error_=sec->ConsultaSecuencial();
                else
                    _error_=sec->ConsultaSecuencial(ops.consulta);
            ...
            {
                _error_=sec->ModificaSecuencial(ops.modifica,ops.key);
                if(!_error_){
                    sec->ConsultaSecuencial(ops.alta);                
                    getch();
                }
    ...
    //Secuencial::ImportaDiccionario("fred.dic","fred.sec");
   /* 
    char* buff;
    void* dato=NULL;
...
Opciones getOpciones(int argc, char** argv){
    ...
    while(*(++argv)){
        ...
        else//selecciona diccionario secuencial            
